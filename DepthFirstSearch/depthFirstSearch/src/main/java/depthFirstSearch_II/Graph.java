package depthFirstSearch_II;
// Programa Java para imprimir DFS
// travessia de um determinado
// gráfico 
import java.util.*;
...
public class Graph {
    ...
    // A função para fazer a travessia DFS. Usa recursiva
    // DFSUtil () 
    void DFS() {
        // Marque todos os vértices como não visitados (defina como
        // falso por padrão em java) 
        boolean visited[] = new boolean[V];
 
        // Chame a função auxiliar recursiva para imprimir DFS
        // travessia a partir de todos os vértices um por um 
        for (int i = 0; i < V; ++i)
            if (visited[i] == false)
                DFSUtil(i, visited);
    }
